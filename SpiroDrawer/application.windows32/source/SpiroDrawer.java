...
public class SpiroDrawer extends PApplet {
...
public void draw(){
  ...
  if(setupMode == 0){
    ...
    ellipse(xPosToPix(wheelx1), yPosToPix(wheely1), PApplet.parseInt(r), PApplet.parseInt(r));
    fill(0, 255, 0);
    ...
    // set wheel 2 center
    fill(200);
    ellipse(xPosToPix(wheelx1), yPosToPix(wheely1), PApplet.parseInt(wheelp1), PApplet.parseInt(wheelp1));
    fill(100);
    ...
    ellipse(xPosToPix(wheelx2), yPosToPix(wheely2), PApplet.parseInt(r), PApplet.parseInt(r));
    fill(0, 255, 0);
    ...
    fill(200);
    ellipse(xPosToPix(wheelx1), yPosToPix(wheely1), PApplet.parseInt(wheelp1), PApplet.parseInt(wheelp1));
    ellipse(xPosToPix(wheelx2), yPosToPix(wheely2), PApplet.parseInt(wheelp2), PApplet.parseInt(wheelp2));
    fill(100);
    ellipse(xPosToPix(wheelx1), yPosToPix(wheely1), 4, 4);
    ellipse(xPosToPix(wheelpx1), yPosToPix(wheelpy1), 4, 4);
  ...
  if(setupMode == 6){
    ...
    float newX = penx;
    float newY = peny;
    speed1 = speedp1*1.0f/speedq1;
    speed2 = speedp2*1.0f/speedq2;
    float tmpx;
    stroke(0, 0, 255);
    
    // iterate 2*PI/stepAngle times for one rotation
    if(speed1 > speed2 && speed1 > 1){
      stepAngle = 2.0f/canvasRadius/speed1;
    }else if(speed2 > speed1 && speed2 > 1){
      stepAngle = 2.0f/canvasRadius/speed2;
    }
...
public float xPixToPos(int p){
  return 1.0f*p - canvasRadius;
}

public float yPixToPos(int p){
  return high*1.0f/2 - p;
}

public int xPosToPix(float p){
  return PApplet.parseInt(p+canvasRadius);
}

public int yPosToPix(float p){
  return PApplet.parseInt(high/2 - p);
}
...
public void keyPressed(){
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "SpiroDrawer" };
    if (passedArgs != null) {
